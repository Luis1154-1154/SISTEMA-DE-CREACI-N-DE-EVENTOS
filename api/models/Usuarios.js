...
exports.getUsuarioByName = (name, callback) => {
  db.query('SELECT id, phone, name, password, role FROM users WHERE name = ?', [name], callback);
};

exports.getUsuarioByPhoneOrName = (phone, name, excludeId, callback) => {
  let sql = 'SELECT id, phone, name, password, role FROM users WHERE (phone = ? OR name = ?)';
